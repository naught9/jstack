...
Host question-tool details: [host-conventions.md](../../references/host-conventions.md). After the first map exists, keep it honest with [maintain-verification-skill](../maintain-verification-skill/SKILL.md).
...
Write `<project-root>/.agents/skills/verify-<app>/SKILL.md` — the consuming project's shared skills tree, even if this generator was loaded from a global jstack install. Create a real directory named `verify-<app>`. Do not write into this jstack checkout's `skills/` inventory, do not overwrite an installer-owned skill symlink, and do not add the generated skill to jstack's `manifest.json`.

Include YAML frontmatter (`name: verify-<app>` and a `description` that names the app, the surface, and when to reach for it — without frontmatter the skill never registers) and these sections, each grounded in what the interview actually found (no placeholders left):
...
Point the user at [maintain-verification-skill](../maintain-verification-skill/SKILL.md) (`/maintain-verification-skill`) for keeping the map honest as the app changes. Suggest a cadence only if they ask.
