...
Don't invent jargon. Use the words a developer would say out loud: "move", "delete", "a budget that only decreases", not "evacuate", "ratchet", or "endgame". A named pattern is fine when the doc says what it means the first time. Add recurring offenders to the prose slop patterns below.
...
- **AI vocabulary.** Additionally, crucial, delve, enduring, enhance, fostering, garner, interplay, intricate, landscape (abstract), pivotal, showcase, tapestry (abstract), testament, underscore, vibrant.
- **Filler and hedging.** "In order to", "due to the fact that", "it is important to note that", "could potentially possibly", "the future looks bright".
- **Fancy "is".** "serves as", "stands as", "boasts", "features" → "is" or "has".
- **Abstract metaphors.** Substrate, wedge, vector, locus, nexus, primitive (as noun), harness (as metaphor), bedrock, scaffolding (as metaphor), modality, paradigm, gold-plating, ratchet, evacuate, endgame, north star, flywheel. Use the concrete word or the real symbol name.
- **Formulaic structure.** "Not just X, but Y." Forced rule-of-three lists. Inline-header bullets that restate the line ("**Performance:** Performance improved...").
- **Chatbot tone.** "I hope this helps!", "Let me know if...", "Great question!", "Certainly!", sycophantic openers.
- **Punctuation tells.** Em dash overuse. Colons that only compare or frame instead of introducing a list.
- **Sterile claims.** Sentences that could appear unchanged in another project's docs. Replace with a mechanism, command, path, or number from this repo.

After the cut, vary rhythm and be specific. Sterile, voiceless writing is still slop.
...
- In Muse repositories, durable engineering docs live in `docs/internal/` (architecture, runbooks, ADRs). Match existing structure and tone there.
- Match the indent style of the file or project you are editing. Write real paths and real symbols. Make every count or tree claim true at the commit that lands it, and include the command that regenerates it.
...
| Audit `docs/internal/` for accuracy and gaps | [docs-audit](../docs-audit/SKILL.md) |
