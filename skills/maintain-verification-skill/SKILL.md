...
A feature map rots the moment the app changes. This skill is the upkeep loop for a skill generated by [create-verification-skill](../create-verification-skill/SKILL.md) (or any project-local verification skill with a feature map). The unit of rigor is the feature, not every sentence: cover every feature file from source and exercise every feature live, without terminalising every bullet.
...
0. **Locate the target.** Find the verification skill to maintain: the project-local skill whose body has launch/drive sections and a feature map (usually `<project-root>/.agents/skills/verify-*/`; also accept `.cursor/skills/verify-*/` if a pstack-era skill already lives there). Several candidates → ask which one; none → stop and point at [create-verification-skill](../create-verification-skill/SKILL.md) instead of inventing a target.
...
2. **Source wave.** One read-only `explorer` subagent per feature file, admitted concurrently. Each explains "how does this user-facing feature work?" from source, flags likely doc drift with citations, and returns one concise live-verification recipe. Children never drive the app and never edit files. Return shape: feature summary / source entry points / likely drift or none / one recipe. Admit the whole wave before collecting results; on asynchronous runtimes, collect an explicit terminal handoff from every child before reconciling. Fall back to the host general-purpose agent only if `explorer` is not installed.
...
6. **Ship or stop.** For changed: one PR of proven corrections (new branch, commit, push, `gh pr create`), re-read every changed file first. For clean or blocked: no PR, report the outcome and the coverage honestly.
