...
Spawn as role `thermo-review` via the host subagent API (see `references/host-conventions.md`). Soft model default: high-reasoning; honor user overrides.

```markdown
You are reviewing the **full stacked epic** for {parent issue} ({title}).

## Task
Run a thermo-nuclear review on the cumulative epic diff. **Review only — do not implement fixes.**
...
Spawn as a general-purpose / implementation worker via the host subagent API. Soft model default: capable/fast; honor user overrides.
