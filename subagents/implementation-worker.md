...
Spawn as a general-purpose / implementation worker via the host subagent API (see `references/host-conventions.md`). Soft model default: capable/fast; honor user overrides.
...
2. Run a **/thermos** review using the thermos skill against this phase's branch changes.
3. Return all findings sorted by severity with file:line references.

## Rules
- You did not implement this phase — review only.
- Do not fix findings; report them for a separate fix subagent.
...
## Findings to address
{paste numbered finding list from review subagent}
