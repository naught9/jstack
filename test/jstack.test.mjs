...
import { CURSOR_SUBAGENT_MODEL, doctor, install } from '../lib/installer.mjs';
...
test('installs every v1 host idempotently and doctor reports healthy assets', async () => {
  ...
  const cursorReviewer = await readFile(path.join(options.projectRoot, '.cursor', 'agents', 'reviewer.md'), 'utf8');
  assert.equal((await lstat(path.join(options.projectRoot, '.cursor', 'agents', 'reviewer.md'))).isSymbolicLink(), false);
  assert.match(cursorReviewer, new RegExp(`^model: ${CURSOR_SUBAGENT_MODEL}$`, 'm'));
  assert.match(cursorReviewer, /Generated by jstack/);
...
test('documents asynchronous Prime fan-out and keeps orchestration skills on the shared contract', async () => {
  ...
  assert.match(thermos, /do not select Opus unless the user named that family/);
...
test('Cursor adapter writes Grok-pinned agent files and replaces legacy symlinks', async () => {
  const options = await fixture({ hosts: new Set(['cursor']) });
  const agentPath = path.join(options.projectRoot, '.cursor', 'agents', 'reviewer.md');
  await mkdir(path.dirname(agentPath), { recursive: true });
  await mkdir(path.join(options.projectRoot, '.agents'), { recursive: true });
  await symlink(path.join(sourceRoot, 'subagents', 'reviewer.md'), agentPath);
  await writeFile(
    path.join(options.projectRoot, '.agents', '.jstack-install.json'),
    `${JSON.stringify({
      version: 1,
      entries: {
        'path:.cursor/agents/reviewer.md': {
          kind: 'symlink',
          target: path.join(sourceRoot, 'subagents', 'reviewer.md'),
        },
      },
    }, null, 2)}\n`,
  );
  ...
  assert.match(content, new RegExp(`^model: ${CURSOR_SUBAGENT_MODEL}$`, 'm'));
  assert.match(content, /Everyday multi-angle review/);
  assert.doesNotMatch(content, /claude-opus/);
});
